...
batch_size = 32
num_of_labels = 4
epochs = 4 # Рекомендуется от 2 до 4
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

### Загрузка данных
df = pd.read_csv(import_training_path, delimiter=',', header=None, names=['label', 'question'])
...
print("Пример вопроса, разбитого на токены:", tokenized_questions[0])
...
# Функция для расчета точности вычислений
...
for _ in trange(epochs, desc="Epoch"):
  ...
  for step, batch in enumerate(train_dataloader):
    ...
    optimizer.zero_grad()
    # Forward pass
    loss = model(b_input_ids, token_type_ids=None, attention_mask=b_input_mask, labels=b_labels-1)
  ...
  print("Точность вычисления - {}".format(eval_accuracy/nb_eval_steps))

# Настройка графика вывода результатов обучения
